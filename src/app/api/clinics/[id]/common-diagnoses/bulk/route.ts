...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    const updatedClinic = await Clinic.findByIdAndUpdate(
      id,
      { $push: { commonDiagnoses: { $each: diagnoses } } },
      { new: true }
    );
    ...
    // Return the newly added diagnoses
    const addedDiagnoses = updatedClinic.commonDiagnoses.slice(-diagnoses.length);
    return NextResponse.json(addedDiagnoses, { status: 201 });
  } catch (error) {
